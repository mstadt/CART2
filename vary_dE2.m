% Script for varying dE2 and seeing how it impacts the model results
clear all;

% Get results for varied dE2 ratios
minratio = 0.1;
maxratio = 5;
nvals = 20;
dE2ratios = linspace(minratio, maxratio, nvals);
...
par_num = 18; % dE2 parameter number
notes = 'try1';
for ii = 1:length(dE2ratios)
    dE2ratio = dE2ratios(ii);
    [T, Y] = vary_res(dE2ratio, par_num);
    fname = strcat('./vary_dE2/',...
                        date,...
                        '_vary_dE2',...
                        '_dE2rat-', num2str(dE2ratio),...
                        '_notes-', notes,...
                        '.mat');
    save(fname, 'T', 'Y', 'dE2ratio')
end
